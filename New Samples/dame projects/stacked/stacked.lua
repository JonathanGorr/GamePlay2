...
tileMapClass = "FlxTilemapStacked";
...
function exportMapCSV( mapLayer, layerFileName )
	...
	--
	stackKeywords = "%zpos%:%tileId%"
	mapText = as3.tolua(DAME.ConvertMapToText(mapLayer,"","\n","",",","","%stackText%",false,false,stackKeywords,";"))
	--print("output to "..as3.tolua(VALUE_CSVDir).."/"..layerFileName)
	DAME.WriteFile(csvDir.."/"..layerFileName.."_stacks.txt", mapText );
end
...
fileText = fileText..tab1.."public class Level_"..levelName.." extends BaseLevelSimple\n"
...
if exportOnlyCSV == false then
	...
	for i,v in ipairs(maps) do
		...
		fileText = fileText..tab3.."layer"..layerName..".loadStackMap( new CSV_"..layerName..", Img_"..layerName..", "..as3.tolua(layer.map.tileWidth)..","..as3.tolua(layer.map.tileHeight)..", "..as3.tolua(layer.map.tileSpacingY)..", "..as3.tolua(layer.map.stackHeight)..", FlxTilemap.OFF, 0, "..as3.tolua(layer.map.drawIndex)..", "..as3.tolua(layer.map.collideIndex)..", new CSV_"..layerName.."_Stacks );\n"
